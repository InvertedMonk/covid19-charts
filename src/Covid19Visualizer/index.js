...
class CovidVisualizer extends React.Component {
  ...
  async changeupperdate(uppdate) {
    ...
    data = data.filter((item) => {
      return item.Date >= this.state.lowerdate && item.Date <= uppdate;
    });
  ...
  async changelowerdate(lowdate) {
    ...
    var data = this.state.countrystats;
    data = data.filter((item) => {
      return item.Date >= lowdate && item.Date <= this.state.upperdate;
    });
          ...
          <Dropdown className="dropdownn">
            <Dropdown.Toggle variant="success" id="dropdown-basic">
              {this.state.global ? (
                ...
                <span>Country-wise Statistics</span>
              )}
            </Dropdown.Toggle>
            ...
            >
              <Dropdown.Item
                key="global"
                onClick={(e) => this.setState({ global: true })}
              >
                Global Statistics
              </Dropdown.Item>
              <Dropdown.Item
                key="countrywise"
                onClick={(e) => this.setState({ global: false })}
              >
          ...
          <>
            <div className="filters">
              <Dropdown>
                ...
                >
                  {this.state.stats.map((countr) => (
                    <Dropdown.Item
                      key={countr.ID}
                      onClick={(e) => this.changecountry(countr, e)}
                    >
                      {countr.Country}
                    </Dropdown.Item>
                  ))}
              ...
              {this.state.country.Country ? (
                <>
                  <Dropdown>
                    <Dropdown.Toggle variant="success" id="dropdown-basic">
                      {this.state.lowerdate ? (
                        <>{this.state.lowerdate.slice(0,10)}</>
                      ) : (
                        <span>Choose Lower Date</span>
                      )}
                    </Dropdown.Toggle>
                    ...
                    >
                      {this.state.countrystats?.map((lowdate) => (
                        <Dropdown.Item
                          key={lowdate?.ID}
                          onClick={(e) => this.changelowerdate(lowdate.Date, e)}
                        >
                          {lowdate.Date.slice(0,10)}
                        </Dropdown.Item>
                      ))}
                    </Dropdown.Menu>
                  </Dropdown>
                  ...
                  <Dropdown>
                    <Dropdown.Toggle variant="success" id="dropdown-basic">
                      {this.state.upperdate ? (
                        <>{this.state.upperdate.slice(0,10)}</>
                      ) : (
                        <span>Choose Upper Date</span>
                      )}
                    </Dropdown.Toggle>
                    ...
                    >
                      {this.state.countrystats?.map((uppdate) => (
                        <Dropdown.Item
                          key={uppdate?.ID}
                          onClick={(e) => this.changeupperdate(uppdate.Date, e)}
                        >
                          {uppdate.Date.slice(0,10)}
                        </Dropdown.Item>
                      ))}
                    </Dropdown.Menu>
                  </Dropdown>
